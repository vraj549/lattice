...
def _prelude(b: CommBuild, setup: dict, meta: dict, mesh_stats: dict,
             need_probes: bool) -> None:
    bolts = _active_bolts(setup, mesh_stats)
    remotes = _active_remotes(setup, mesh_stats)
    ...
    for rr in remotes:
        _env_sphere(b, f"RPN{rr['load_index']}", rr["node"], 1e-5)
    b.w()

    beam_groups = ([f"BOLT{br['index']}" for br in bolts]
                   + [f"RPT{rr['load_index']}" for rr in remotes])
    ...
    if remotes:
        ...
        for rr in remotes:
            affes.append(f"_F(GROUP_MA=('RPT{rr['load_index']}',), MATER=matrp)")
    b.w()
    ...
    if beam_groups:
        ...
        for rr in remotes:
            b.w(f"    _F(GROUP_MA=('RPT{rr['load_index']}',), SECTION='CERCLE', "
                f"CARA='R', VALE=0.5),")
        b.w("))")
        b.w()
        ...
        for rr in remotes:
            b.w(f"    _F(GROUP_NO_MAIT='RPN{rr['load_index']}',")
            b.w("       DDL_MAIT=('DX', 'DY', 'DZ', 'DRX', 'DRY', 'DRZ'),")
            b.w(f"       GROUP_NO_ESCL=('LOA{rr['load_index']}',),")
            b.w("       DDL_ESCL=('DX-DY-DZ',), COEF_ESCL=(1.0,)),")
        b.w("))")
        b.w()
...
def _active_remotes(setup: dict, mesh_stats: dict) -> list:
    recs = {r.get("id"): r for r in mesh_stats.get("remotes", [])}
    out = []
    n_remote_setup = 0
    for l in setup.get("loads", []):
    ...
    if n_remote_setup != len(mesh_stats.get("remotes", [])):
        raise ValueError("Mesh is older than the remote-load setup — re-mesh before solving.")
    return out
...
def _supports(b: CommBuild, setup: dict) -> str:
    ...
    if not ddl_items and not face_items:
        raise ValueError("No supports defined — add at least one support with faces.")
...
def _loads(b: CommBuild, setup: dict, meta: dict, var: str = "loadc",
        ...
        if t == "force" and l.get("faces"):
            ...
            for c, key in (("FX", "fx"), ("FY", "fy"), ("FZ", "fz"),
                ...
                if v != 0.0:
                    comps.append(f"{c}={_fmt(v)}")
            if comps:
                nodal_items.append(f"_F(GROUP_NO=('RPN{i + 1}',), {', '.join(comps)})")
...
def _support_groups(setup: dict) -> list:
    return [f"SUP{i + 1}" for i, s in enumerate(setup.get("supports", [])) if s.get("faces")]


# --------------------------------------------------------------------------

def write_static(setup: dict, meta: dict, mesh_stats: dict, cfg: dict) -> CommBuild:
    b = CommBuild()
    _prelude(b, setup, meta, mesh_stats, need_probes=False)
    fix = _supports(b, setup)
    load = _loads(b, setup, meta)
...
def write_harmonic(setup: dict, meta: dict, mesh_stats: dict, cfg: dict) -> CommBuild:
