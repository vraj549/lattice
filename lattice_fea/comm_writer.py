...
def _prelude(b: CommBuild, setup: dict, meta: dict, mesh_stats: dict,
    ...
    # ERREUR_F='EXCEPTION' is what makes every try/except below this line mean
    # anything. code_aster's default is ABORT: a fatal <F> error terminates the
    # process from Fortran, so the Python handler never runs, FIN() is never
    # called, and the result files are never flushed. A run that had already
    # computed its modes and failed on one post-processing table therefore lost
    # the modes too — which is the incident this module's docstring describes
    # the try/except as the fix for. It was not the fix; this is.
    b.w("DEBUT(LANG='EN', ERREUR=_F(ERREUR_F='EXCEPTION'))")
    b.w()
...
DEBUT_LINE = "DEBUT(LANG='EN', ERREUR=_F(ERREUR_F='EXCEPTION'))"


def _finish(b: CommBuild) -> None:
    """Close the deck so a failure still flushes whatever was computed.

    Everything after DEBUT is wrapped, because FIN() is what closes the MED
    and the table files. Skipping it — which is what an uncaught error does —
    leaves a result that exists in memory and nowhere on disk. With the body
    guarded, a run that dies after IMPR_RESU still hands back the fields it
    wrote, and the job is still reported as failed with the original error.

    The failure is re-raised, never swallowed. A partial result is offered for
    inspection, not presented as a finished one; the exit code stays non-zero
    and `meta["exit_code"]` carries it into the UI.
    """
    try:
        head = b.lines.index(DEBUT_LINE) + 1
    except ValueError:                      # pragma: no cover - deck shape changed
        b.w("FIN()")
        return
    body = b.lines[head:]
    b.lines = b.lines[:head] + [
        "",
        "# Guarded so that FIN() below is reached even when something here",
        "# fails: FIN() is what flushes the result files to disk.",
        "_lattice_failure = None",
        "try:",
    ] + [f"    {ln}" if ln else "" for ln in body] + [
        "except Exception as _lattice_e:",
        "    import traceback",
        "    traceback.print_exc()",
        "    _lattice_failure = _lattice_e",
        "    print('Lattice: the run failed -', _lattice_e)",
        "",
        "if _lattice_failure is None:",
        "    FIN()",
        "else:",
        "    try:",
        "        FIN()",
        "    except Exception as _lattice_fin:",
        "        print('Lattice: FIN() after the failure -', _lattice_fin)",
        "    raise _lattice_failure",
    ]
